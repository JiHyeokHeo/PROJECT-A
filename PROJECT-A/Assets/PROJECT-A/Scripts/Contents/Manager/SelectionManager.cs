...
namespace A
{
    ...
    {
        public readonly List<ISelectable> selectables = new List<ISelectable>(); // 전체 후보 등록
        public readonly List<ISelectable> current = new List<ISelectable>();     // 현재 선택된 친구들
        ...
        float edgeSize = 10f; // 가장자리 감지 영역 px
        float moveSpeed = 10f;
        ...
        private void OnGUI()
        {
            ...
            //string text2 = string.Format("Selected Count ({0:0.0} 개)", current.Count);
